...
export function CanvasToolbar({ onAddNode }: CanvasToolbarProps) {
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`실행 오류: ${msg}`)
    } finally {
      setIsExecuting(false)
    }
  }, [isExecuting, setIsExecuting, clearNodeOutputs, setNodeOutput, t])
    ...
    <div className="absolute top-2 left-2 z-20 flex items-start gap-2">
      ...
      <button
        ...
        className={cn(
          ...
          isExecuting
            ? 'bg-yellow-500/20 border-yellow-500/40 text-yellow-500 cursor-not-allowed'
