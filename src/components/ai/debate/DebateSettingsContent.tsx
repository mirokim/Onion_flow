/**
 * DebateSettingsContent — Debate configuration UI for SettingsDialog.
 * All debate settings (mode, participants, roles, rounds, pacing, reference) live here.
 */
...
import type { AIProvider, DiscussionMode, ReferenceFile } from '@/types'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        {enabledProviders.length < 2 && (
          <div className="flex items-center gap-2 text-warning text-xs bg-warning/10 px-3 py-2 rounded-lg">
            ...
            <span>AI 설정에서 2개 이상의 AI를 활성화하고 API 키를 입력하세요</span>
          </div>
        )}
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <button
            onClick={() => updateSettings({ pacingMode: 'auto' })}
            className={cn(
              'px-3 py-2 text-xs rounded-lg border transition-all',
          ...
          <button
            onClick={() => updateSettings({ pacingMode: 'manual' })}
            className={cn(
              'px-3 py-2 text-xs rounded-lg border transition-all',
